...
/*
 * Init and run calls for RTL_HOME flight mode
 *
...
bool ModeRTL_HOME::init(bool ignore_checks)
{
    if (!ignore_checks) {
...
void ModeRTL_HOME::restart_without_terrain()
{
...
ModeRTL_HOME::RTLAltTypeHome ModeRTL_HOME::get_alt_type() const
{
    // sanity check parameter
    switch ((ModeRTL_HOME::RTLAltTypeHome)g.rtl_alt_type_home) {
    case RTLAltTypeHome::RTL_ALTTYPE_RELATIVE_HOME ... RTLAltTypeHome::RTL_ALTTYPE_TERRAIN_HOME:
        return g.rtl_alt_type_home;
    }
    // user has an invalid value
    return RTLAltTypeHome::RTL_ALTTYPE_RELATIVE_HOME;
}

// rtl_run - runs the return-to-launch controller
// should be called at 100hz or more
void ModeRTL_HOME::run(bool disarm_on_land)
{
    if (!motors->armed()) {
        return;
    }
...
void ModeRTL_HOME::return_start()
{
...
void ModeRTL_HOME::climb_return_run()
{
...
void ModeRTL_HOME::loiterathome_start()
{
...
//      called by rtl_run at 100hz or more
void ModeRTL_HOME::loiterathome_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        make_safe_ground_handling();
...
void ModeRTL_HOME::descent_start()
{
...
bool ModeRTL_HOME::is_landing() const
{
...
// called by rtl_run at 100hz or more
void ModeRTL_HOME::land_run(bool disarm_on_land)
{
    // check if we've completed this stage of RTL
...
// #endif
    ...
    if (terrain_following_allowed && (get_alt_type() == RTLAltTypeHome::RTL_ALTTYPE_TERRAIN_HOME)) {
...
bool ModeRTL_HOME::get_wp(Location& destination) const
{
...
uint32_t ModeRTL_HOME::wp_distance() const
{
    return wp_nav->get_wp_distance_to_destination();
}

int32_t ModeRTL_HOME::wp_bearing() const
{
    return wp_nav->get_wp_bearing_to_destination();
}

// returns true if pilot's yaw input should be used to adjust vehicle's heading
bool ModeRTL_HOME::use_pilot_yaw(void) const
{
...
bool ModeRTL_HOME::set_speed_xy(float speed_xy_cms)
{
    copter.wp_nav->set_speed_xy(speed_xy_cms);
    return true;
}

bool ModeRTL_HOME::set_speed_up(float speed_up_cms)
{
    copter.wp_nav->set_speed_up(speed_up_cms);
    return true;
}

bool ModeRTL_HOME::set_speed_down(float speed_down_cms)
{
